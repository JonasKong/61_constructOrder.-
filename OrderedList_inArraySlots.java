...
public class OrderedList_inArraySlots
    ...
    public OrderedList_inArraySlots
		...
		for (int transferred = 0; transferred < unordered.size(); transferred ++){
			list_iAS.add(unordered.get(champIndex(unordered)));
			unordered.set(champIndex(unordered), null);
		}
    }
     ...
     */
     private int champIndex( ArrayList<Integer> challengers) {
		int initialIndex = 0;
		while (challengers.get(initialIndex) == null) {
			initialIndex ++;
		}
		int currentLow = challengers.get(initialIndex);
		int indexAt = initialIndex;
		for (int index = 0; index < challengers.size(); index ++) {
			if (challengers.get(index) != null) {
				if (challengers.get(index) < currentLow) {
					currentLow = challengers.get(index);
					indexAt = index;
				}
			}
		}
